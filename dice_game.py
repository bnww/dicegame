...
class DiceGame:
    ...
    def get_next_states(self, action, dice_state):
        ...
        if count == self._dice:
            ...
            other_vals = np.array(list(itertools.combinations_with_replacement(self._values, self._dice - count)), dtype=int)
            ...
            other_index = np.array(list(itertools.combinations_with_replacement(range(self._sides), self._dice - count)), dtype=int)
            ...
            other_vals = np.insert(other_vals, np.zeros(count, dtype=int), np.asarray(dice_state, dtype=int)[mask], axis=1)
